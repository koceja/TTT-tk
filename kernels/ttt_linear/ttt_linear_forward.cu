...
template <int a_x, int a_y, int b_x, int b_y>
__device__ static inline void matmul(
    rt_base<float, ducks::rt_layout::row> &d,
    const rt_base<float,  ducks::rt_layout::row> &a,
    const rt_base<float,  ducks::rt_layout::col> &b, // in col-major mode
...
)
{
    ...
    rt_bf<a_x, a_y, ducks::rt_layout::row> a_bf;
    rt_bf<b_x, b_y, ducks::rt_layout::col> b_bf;
...
)
{
    ...
    const kittens::coord idx1{0, blockIdx.x, 0, 0};

    // printf("Hello from the device! Thread ID: %d\n", threadIdx.x);

    kittens::load(W1_init, W1_init_gl, {blockIdx.x, 0, 0});
    kittens::load(b1_init, b1_init_gl, idx1);

    kittens::load(W1_init, ttt_norm_weight_gl, {blockIdx.x, 0, 0});
    kittens::load(b1_init, ttt_norm_bias_gl, idx1);
    ...
    rt_fl<16, F> ln_w_reg;
    rt_fl<16, F> ln_b_reg;
    load(ln_w_reg, ttt_norm_weight_gl, ln_w_reg.cols);
    load(ln_b_reg, ttt_norm_bias_gl, ln_b_reg.cols);
    ...
    for (int i = 0; i < NC; i++) {
        ...
        load(XV_smem[0],  XV_batch_gl, {blockIdx.x, i, 0, 0});
        load(XK_smem[0],  XK_batch_gl, {blockIdx.x, i, 0, 0});
        load(XQ_smem[0],  XQ_batch_gl, {blockIdx.x, i, 0, 0});
        load(Eta_smem[0], eta_batch_gl, {blockIdx.x, i, 0, 0});
        ...
        // rt_fl<16, F> Z1_reg;
        // matmul<16, 64, 64, 64>(Z1_reg, XK_reg, W1_init, b1_init);
...
void run_benchmark()
{
    ...
    const auto run_kernel = [&]()
    {
        ...
        ttt_linear_forward_kernel<<<B * NH, threads, NC*CS*F*3>>>(
            W1_init_gl,
            b1_init_gl,
            XQ_batch_gl,
            XV_batch_gl,
